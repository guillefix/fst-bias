...
from FAdo import common
from FAdo.fa import DFA, NFA, statePP
from FAdo.transducers import SFT, GFT, Transducer
...
class ParserFAdo(Yappy):
    ...
    def firstTransitionTransSemRule(self, lst, context=None):
        ...
        if self.TRtype is None and (lst[1] != common.Epsilon and len(lst[1]) > 1) or (lst[2] != common.Epsilon and len(
            lst[
            2]) > 1):
            self.TRtype = "GFT"
        ...
        for sin in tr.delta:
            for syin in tr.delta[sin]:
                for (syout, sout) in tr.delta[sin][syin]:
                    io.write("{0:>s} {1:>s} {2:>s} {3:>s}\n".format(statePP(tr.States[sin]),statePP(tr.States[sout]), str(syin), str(syout)))
        # io.write("\n")
        for s in tr.Final:
            io.write("{0:>s}\n".format(statePP(tr.States[s])))
